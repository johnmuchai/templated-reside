...
//echo encryptIt($_POST['password']);
			...
			$mngCheck = $mysqli->query("SELECT 'X' FROM manager_users WHERE email = '".$emailCheck."'");
			if ($mngCheck->num_rows) {
				$isManager = 'true';
			}
			...
			if ($isUser == 'true') {
				...
				if ($trow['isArchived'] == '1') {				// If the account is Archived - Do NOT allow the login
					...
					if (!empty($userId)) {
						...
						$_SESSION['rs']['isManager']		= $isManager;

						if($isManager=='true'){

							$qry = "SELECT * FROM manager_users WHERE email = '".$emailCheck."'";
							$results = mysqli_query($mysqli, $qry) or die('-2' . mysqli_error());
							$row = mysqli_fetch_assoc($results);
							$_SESSION['rs']['managerId']	 = $row['manager_id'];
					...
					//echo "Error doing query ".mysqli_error($mysqli);
				}
					...
					<div class="signin">
						...
						<div class="signin-form">
							...
							<form action="sign-in.php" method="post" class="mt-20">
								...
								<p class="text-center">
									<button type="submit" name="submit" value="signIn" class="btn btn-success btn-lg btn-block btn-icon"><i class="fa fa-sign-in"></i> <?php echo $signInText; ?></button>
								</p>
							</form>
						</div>
