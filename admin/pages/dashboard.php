...
if ($set['enablePayments'] == '1') {
	...
	payments.rentYear = '".$currentYear."' limit 10";
	...
	$avail = "SELECT * FROM properties WHERE isLeased = 0 limit 10";
	$availres = mysqli_query($mysqli, $avail) or die('-11' . mysqli_error());

	//Get overdue accounts

	$overdue = "SELECT accounts.balance,users.userFirstName,users.userLastName,properties.propertyName,properties.unitName,properties.propertyId,users.userId FROM accounts
	INNER JOIN leases on accounts.leaseId=leases.leaseId
	INNER JOIN properties on leases.propertyId=properties.propertyId
	INNER JOIN users on users.userId=accounts.tenantId where accounts.balance<0 order by accounts.balance asc limit 10";

	$overdueres = mysqli_query($mysqli, $overdue) or die('-11' . mysqli_error());
		...
		<div class="container page_block mt-20">
			...
			<?php if(mysqli_num_rows($latepayres) > 0) { ?>
				<table id="overdueRent" class="display" cellspacing="0">
					...
					<tbody><?php
					while ($ov = mysqli_fetch_assoc($overdueres)) {
						?>
						<tr>
							<td>
								<a href="index.php?action=viewParent&propertyId=<?php echo $ov['propertyId']; ?>" data-toggle="tooltip" data-placement="top" title="<?php echo $viewPropertyText; ?>">
									<?php echo clean($ov['propertyName']); ?>
								</a>
							</td>
							<td><?php echo clean($ov['unitName']); ?></td>
							<td>
								<a href="index.php?action=viewTenant&userId=<?php echo $ov['userId']; ?>" data-toggle="tooltip" data-placement="top" title="<?php echo $viewTenantText; ?>">
									<?php echo clean($ov['userFirstName']).' '.clean($ov['userLastName']); ?>
								</a>
							</td>
							<td><?php echo formatCurrency($ov['balance']); ?></td>
						</tr>
					<?php } ?>
