<?php
		...
		$activityType = '2';
		$rs_uid = '0';
		$activityTitle = $rs_adminName.' viewed the Payment Receipt for "'.clean($row['propertyName']).'"';
		updateActivity($rs_adminId,$rs_uid,$activityType,$activityTitle);
	} else {
		// Add Recent Activity
		$activityType = '21';
		$rs_uid = '0';
		$activityTitle = $rs_adminName.' directly accessed a Payment Receipt';
		updateActivity($rs_adminId,$rs_uid,$activityType,$activityTitle);
	}
?>
<!DOCTYPE html>
			...
			<div class="row">
				<div class="col-xs-4 col-xs-offset-8">
					<p class="mb-20 text-right">
						...
						<?php if ($row['isRent'] == '1') { ?>
							<br /><?php echo $rentMonthText; ?>: <?php echo clean($row['rentMonth']).' '.clean($row['rentYear']); ?>
						<?php } ?>
					</p>
				</div>
			</div>
